...
@app.route("/users/<int:task_id>/delete/", methods=["POST"])
@login_required
...
def master_determine(recruiter_id):
    ...
    return render_template('master/master_homepage.html', recruiter=recruiter, applicant=applicant, companies=companies, interviews=interviews, jobs=jobs, apply=apply, task=task, call_recruiter=call_recruiter)
...
@app.route("/master/<int:applicant_id>/delete/", methods=["POST"])
@login_required
...
def task_delete_master(task_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flash("Task deleted successfully.", "success")
        return redirect(url_for('master_determine'))


@app.route("/master/<int:company_id>/delete/", methods=["POST"])
@login_required
...
@app.route("/master/<int:job_id>/delete/", methods=["POST"])
@login_required
